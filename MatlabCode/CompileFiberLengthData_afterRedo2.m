...
for i=1:length(birdNums)
    ...
    for j=1:length(tempLG)
        ...
        if tempLG(j).avLen>2
            factor = 10;  % was measured in cm
        else
            factor = 25.4  % was measured in inchest
        end
        fl(i).LG(j).avLen = factor*tempLG(j).avLen;
        fl(i).LG(j).sdLen = std(factor*[tempLG(j).length]);
    ...
    for j=1:length(tempMG)
        ...
        fl(i).MG(j).avLen = 25.4*tempMG(j).avLen;
        fl(i).MG(j).sdLen = std(25.4*[tempMG(j).length]);  % the Mg was measured in inches
...
save('Data\CompliedFLbyBird2.mat','fl')
...
% Import the data
LgSarcomereLengthsSmcafterRedo4 = readtable("Data\LgSarcomereLengthsSmc_afterRedo4.xlsx", opts, "UseExcel", false);
...
% Import the data
MGSarcomereLengthsSmcafterRedo = readtable("Data\MGSarcomereLengthsSmc_afterRedo.xlsx", opts, "UseExcel", false);


% Clear temporary variables
clear opts = readtable("Data\MGSarcomereLengthsSmc_afterRedo.xlsx", opts, "UseExcel", false);
...
clear
load('Data\CompliedFLbyBird2.mat')
...
% Import the data
tbl = readtable("Data\MsucleTendonPathDataCompiled.xlsx", opts, "UseExcel", false);
...
folder = 'Data\UpdatedPennationData';
files = dir(folder);
