function []= writetrc_zmc(markers,MLabels,VideoFrameRate,FullFileName)   
%
...
Time = ((0:1/VideoFrameRate:(length(markers(:,1))-1)*(1/VideoFrameRate))');
 tidx =0:(length(markers(:,1))-1);
 tsec = (tidx/VideoFrameRate)';
DataStartFrame=Time(1)*VideoFrameRate+1;

%add frame column
frameArray=(1:length(markers(:,1)))';

markers=[frameArray Time markers];
MLabels = [MLabels];
...
for i= 1:nFrames
    for j=1:ncol
        ...
        elseif j == 2
            ...
            fprintf(fid,'%f\t',markers(i,j));   % convert to meters
        end
    end
    fprintf(fid,'\n');
end
